...
function savePublication(req, res) {
	...
	publication.save((err, publicationStored) => {
		...
		if (!publicationStored) return res.status(404).send({message: 'La publicacion no ha sido guardado'});
...
//Devolvera las publicaciones de los usuarios q sigo
function getPublications(req, res) {
	...
	Follow.find({user: req.user.sub}).populate('followed').exec((err, follows) => {
		...
		Publication.find({user: {"$in": follows_clean}}).sort('-created_at').pupulate('user').paginate(page, itemsPerPage, (err, publications, total) => {
...
//subir archivos de imagen/avatar de publicaciones
